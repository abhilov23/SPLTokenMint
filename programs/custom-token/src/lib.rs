...
pub mod custom_token {
    ...
    pub fn create_token_with_extensions(
        ...
        if args.require_memo {
            let memo_ix = enable_required_transfer_memos(
                &ctx.accounts.token_program.key(),
                &ctx.accounts.mint.key(),
                &ctx.accounts.mint_authority.key(),
                &[],
        ...
        // Step 8: Initialize Immutable Owner (conditionally)
        ...
        // Step 9: Mint initial supply if specified
        if args.initial_supply > 0 {
            // Check if the recipient_ata is valid
            ...
            // Create ATA if it's empty (i.e., doesn't exist yet)
...
// Helper function moved outside the program module to fix the compilation issue
